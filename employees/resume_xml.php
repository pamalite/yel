...
if ($GLOBALS['protocol'] == 'https') {
    if (empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] == 'off') {
        redirect_to('https://'. $GLOBALS['root']. '/employees/resume_xml.php?id='. $_GET['id']);
        exit();
    }
}
...
$resume_data['resume']['DISCLAIMER_NOTE'] = 'Generated from YellowElevator.com.';
...
if (count($experiences) > 0) {
    ...
    foreach($experiences as $experience) {
        ...
        $resume_data['resume']['work_experiences']['work_experience'][$i]['reason_for_leaving'] = $experience['reason_for_leaving'];
