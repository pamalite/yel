...
CREATE TABLE `referral_buffers` (
  ...
  `notes` mediumtext,
  PRIMARY KEY (`id`),
  UNIQUE KEY `referrer_email` (`referrer_email`,`candidate_email`,`job`),
  FULLTEXT KEY `resume_file_text` (`resume_file_text`),
  FULLTEXT KEY `testimony` (`notes`),
  FULLTEXT KEY `referrer_name` (`referrer_name`),
  FULLTEXT KEY `candidate_name` (`candidate_name`),
  FULLTEXT KEY `criteria_match` (`notes`,`resume_file_text`,`referrer_name`,`candidate_name`)
...
CREATE TABLE `referral_index` (
  ...
  FULLTEXT KEY `notes_2` (`notes`,`testimony`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2010-06-20 21:38:06
