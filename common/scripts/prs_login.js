...
function login() {
    ...
    var login_uri = root + "/prs/login_action.php";
    ...
    var request = new Request({
        ...
        onSuccess: function(txt, xml) {
            ...
            if (xml.getElementsByTagName('errors').length != 0) {
                ...
                if (msg == "bad_login") {
                    location.replace(root + '/errors/failed_login.php?dir=prs');
                }
            ...
            if (status[0].childNodes[0].nodeValue == 'ok') {
                location.replace(root + '/prs/home.php');
            }
        },
...
function get_seed() {
    var seed_uri = root + "/prs/seed.php";
    var request = new Request({
        url: seed_uri,
        onSuccess: function(txt, xml) {
            set_status("");
